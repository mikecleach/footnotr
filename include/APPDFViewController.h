...
/**
 * Convenience method to reload the PDF view with a different document.
 *
 * \param pdf The new PDF File to present in the view controller.
 *
 * \param preserveLayout Pass YES to have the new document reload at
 * the same place, NO to reload on the first page like normal. Only
 * set this to YES if you are reloading the same (or similar)
 * document, e.g., after a document assembly operation.
 */
-(void)reloadWithPDF:(APPDFDocument *)pdf preserveLayout:(BOOL)preserveLayout;
...
/**
 * DEPRECATED: no longer relevant since iPads can now do mirroring
 * natively. This method will be removed in a future version of the
 * SDK.
 *
 ...
 */
-(UIViewController *)mirroringController __attribute__((deprecated));
 ...
 */
-(void)setMirroringControllerViewSize:(CGSize)size __attribute__((deprecated));
 ...
 * view heirarchy.
 */
-(void)releaseMirroringController __attribute__((deprecated));
